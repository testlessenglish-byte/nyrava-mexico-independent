-- Bind approval to every reviewed report column, not only full_report.
-- Keep existing argument names; old eight-argument calls resolve the default
-- but cannot release without supplying the new expected snapshot.
drop function if exists public.finalize_report_release(uuid,uuid,uuid,jsonb,jsonb,boolean,jsonb,text);
...
$$;
revoke all on function public.finalize_report_release(uuid,uuid,uuid,jsonb,jsonb,boolean,jsonb,text,jsonb) from public;
grant execute on function public.finalize_report_release(uuid,uuid,uuid,jsonb,jsonb,boolean,jsonb,text,jsonb) to authenticated, service_role;
