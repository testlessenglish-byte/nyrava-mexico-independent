...
function fakeDb(rows: Row[]) {
  ...
  const db = {
    from() {
      const state: { updates?: Record<string, unknown>; filters: Array<[string, unknown]>; members?: [string, unknown[]]; lt?: [string, string] } = {
        filters: [],
      };
      const builder: Record<string, unknown> = {
        ...
        then(res: (v: { data: Row[]; error: null }) => unknown) {
          ...
          if (state.members) out = out.filter(r => state.members![1].includes(r[state.members![0]]));
...
describe("full stall sequence: checkpoint -> requeue -> late heartbeat -> sweeper recovery", () => {
  it("does not sweep engine rows while a long report has a live worker lease", async () => {
    ...
    const db = { from(table: string) {
      ...
      const q: any = {
        select: () => q, lt: () => q, in: () => q,
        eq: (column: string, value: unknown) => { if (column === "status" && value === "queued") queued = true; return q; },
        then: (resolve: any) => Promise.resolve(resolve({ data: queued ? [] : [{ id: "long-report", updated_at: old, worker_lease_until: future }], error: null })),
      };
      return q;
    } };
    expect(await sweepStalledCases(db as never)).toEqual({ swept: 0, autoRetried: 0, ids: [] });
    expect(touched).not.toContain("pipeline_engine_runs");
  });
