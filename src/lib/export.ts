...
import { NYRAVA_N_LOGO_BASE64 } from "./pdf/logo-asset";
import { NYRAVA_APPROVED_COVER_BASE64 } from "./pdf/cover-asset";

// Load the authentic high-resolution Nyrava "N" logo asset
function getLogoBase64(): Promise<string | null> {
  return Promise.resolve(NYRAVA_N_LOGO_BASE64);
}
import {
  rt,
...
const BRAND_CYAN: [number, number, number] = [...PDF_THEME.colors.goldLight];
...
/** Source-page line endings are extraction layout, not paragraph breaks in a
 * quotation. Reflow the entire passage; never cut it mid-word at a char cap. */
export function formatPdfSourceQuote(value:string):string {
...
let _annexCitations: Array<Record<string, any>> = [];
...
// Print-oriented palette: deep Nyrava forest green (#104033) + muted gold accent (#C49257) on a warm
// ivory / cream sheet (#FAF6F0). Deliberately not pure white / cool slate, and zero purple —
// the warm paper tone and forest green header are what make the export read as an attorney-grade
// Mexican legal intelligence work product.
const PAGE_BG: [number, number, number] = [...PDF_THEME.colors.pageBg]; // warm ivory/cream sheet (#FAF6F0)
const PRIMARY: [number, number, number] = [...PDF_THEME.colors.primary]; // deep Nyrava green (#104033)
const PRIMARY_DEEP: [number, number, number] = [...PDF_THEME.colors.primaryDark]; // dark forest green (#0D281E)
const ACCENT: [number, number, number] = [...PDF_THEME.colors.gold]; // muted gold (#C49257)
const ACCENT_SOFT: [number, number, number] = [...PDF_THEME.colors.goldLight]; // pale gold (#D4AF7A)
const INK: [number, number, number] = [...PDF_THEME.colors.ink]; // deep dark pine ink (#1C2621)
const MUTED: [number, number, number] = [...PDF_THEME.colors.muted]; // restrained slate (#62685F)
const LINE: [number, number, number] = [...PDF_THEME.colors.line]; // hairlines on light bg (#DED4C2)
const SUCCESS: [number, number, number] = [...PDF_THEME.colors.success];
const DANGER: [number, number, number] = [...PDF_THEME.colors.danger];
const HIGH: [number, number, number] = [...PDF_THEME.colors.high];
const MEDIUM: [number, number, number] = [...PDF_THEME.colors.medium];
const QUOTE_BG: [number, number, number] = [...PDF_THEME.colors.quoteBg]; // evidence blockquote fill (#F7F2EB)
const CARD_BG: [number, number, number] = [...PDF_THEME.colors.cardBg]; // card fill
const CARD_BORDER: [number, number, number] = [...PDF_THEME.colors.cardBorder]; // card border (#E5DACD)
...
function renderDecisionCore(b: PdfBuilder, data: CaseExportData) {
  for (const section of presentation(data).decision_sections) {
    ...
    b.text(section.speaker_label, { size: 9, color: MUTED, gap: 5 });
  }
...
const NAVY_TINT: [number, number, number] = [...PDF_THEME.colors.navy]; // deep navy accent (#0F2B38)
const SILVER: [number, number, number] = [...PDF_THEME.colors.goldLight]; // pale gold ring (#D4AF7A)
const SHIELD_DARK: [number, number, number] = [...PDF_THEME.colors.primary]; // brand pine green plate (#104033)
...
export class PdfBuilder {
  ...
  logoBase64: string | null = NYRAVA_N_LOGO_BASE64;
  ...
  // Restrained semantic attribution badges for legal speakers
  attributionBadge(
    roleLabel: string,
    x?: number,
    y?: number,
    ...
    let textColor = PDF_THEME.colors.tribunalBadge;
    let bgColor = PDF_THEME.colors.tribunalBadgeBg;
    if (s.includes("tribunal") || s.includes("resolutiv") || s.includes("juzgado") || s.includes("corte")) {
      textColor = PDF_THEME.colors.tribunalBadge;
      bgColor = PDF_THEME.colors.tribunalBadgeBg;
    } else if (s.includes("parte") || s.includes("alegaci") || s.includes("actor") || s.includes("demandad")) {
      textColor = PDF_THEME.colors.parteBadge;
      bgColor = PDF_THEME.colors.parteBadgeBg;
    } else if (s.includes("no verificad") || s.includes("sin verificar") || s.includes("pendiente") || s.includes("riesgo")) {
      textColor = PDF_THEME.colors.unverifiedBadge;
      bgColor = PDF_THEME.colors.unverifiedBadgeBg;
    }
    ...
    const w = tw + padX * 2;
    const h = 12;
    const posX = x ?? this.margin;
    const posY = y ?? this.y;
    this.doc.setFillColor(...bgColor);
    this.doc.roundedRect(posX, posY, w, h, 2.5, 2.5, "F");
    this.doc.setTextColor(...textColor);
    this.doc.text(label, posX + padX, posY + 8.5);
    if (!y) this.y += h + 6;
    return w;
  }
  ...
  sectionTitle(kicker: string, title: string): number {
    ...
    // Continuation labels are drawn once in header(), never once per section
    // sharing a page (which stacked several labels at the same coordinates).
  ...
  evidenceQuote(text: string, attribution = "") {
    text = formatPdfSourceQuote(text);
    if (!text) return;
  ...
  header() {
    ...
    const bandH = PDF_THEME.dimensions.runningHeaderBandH; // 36
    let lastSection = "";
    for (let i = 2; i <= pageCount; i++) {
      this.doc.setPage(i);

      // Warm sheet across the reserved top area
      this.doc.setFillColor(...PAGE_BG);
      ...
      this.doc.setTextColor(255, 255, 255);
      this.doc.text("NYRAVA MÉXICO", textX, markCy - 1);
      this.doc.setFont("helvetica", "normal");
      ...
      if (currentSecTitle) {
        ...
        this.doc.setFontSize(7.5);
        this.doc.setTextColor(255, 255, 255);
        const centerTitle = currentSecTitle.toUpperCase();
        const centerMaxW = 160;
        const centerFitted =
          (this.doc.splitTextToSize(centerTitle, centerMaxW) as string[])[0] ?? centerTitle;
        this.doc.text(centerFitted, this.pageW / 2, markCy + 2.5, { align: "center" });
      }

      // Right side: short matter ID + page stamp
      this.doc.setFont("helvetica", "normal");
      this.doc.setFontSize(7.5);
      this.doc.setTextColor(...ACCENT_SOFT);
      const shortId = this.matterId ? (this.matterId.length > 20 ? this.matterId.slice(0, 18) + "..." : this.matterId) : "";
      const pageWord = getReportTemplateLocale() === "en" ? "Page" : "Página";
      const rightLabel = shortId ? `${shortId}   ·   ${pageWord} ${i} / ${pageCount}` : `${pageWord} ${i} / ${pageCount}`;
      const rightMaxW = 180;
      const fitted =
        (this.doc.splitTextToSize(rightLabel, rightMaxW) as string[])[0] ?? rightLabel;
      this.doc.text(fitted, this.pageW - this.margin, markCy + 2.5, { align: "right" });

      // Thin muted gold rule separating header band from content
      this.doc.setDrawColor(...ACCENT);
      this.doc.setLineWidth(0.75);
      this.doc.line(this.margin, bandH + 2, this.pageW - this.margin, bandH + 2);
  ...
  footer(meta: { parity: string; ess: string; generatedAt: string } | null) {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Hairline above the footer row in muted gold
      if (i > 1) {
        this.doc.setDrawColor(...LINE);
        this.doc.setLineWidth(0.6);
      ...
      const brandLine = `NYRAVA MÉXICO  ·  mexico.nyrava.com  ·  ${this.matterId || this.caseName}`;
...
function renderCover(
  ...
  b.premiumCover({
    ...
    court: translateLegalTerm(reportCourt),
    ...
    return {
      checklistText:
        locale === "en"
          ...
          : "El análisis se limita a lo que consta en la resolución aportada. Sus antecedentes describen hechos según el tribunal; no sustituyen las pruebas originales. La verificación independiente de esos hechos, la reconstrucción íntegra del procedimiento o la preparación de una nueva promoción pueden requerir las constancias citadas que no se hayan aportado. El resultado sólo se confirma cuando se localizan los puntos resolutivos.",
    };
  }
...
function renderExecutive(b: PdfBuilder, data: CaseExportData, mode: ReportMode) {
  ...
  if (mode === "LIMITED") {
    ...
    b.h2(execLocale === "en" ? "Scope of review" : "Alcance de la revisión");
    b.text(execLocale === "en"
  ...
  for (const p of rows) {
    ...
    for (const line of lines) {
      ...
      if (Object.keys(citation).length) {
        b.text(
          `Citation: "${formatPdfSourceQuote(asStr(citation.quote))}" — ${citeLabel(citation.doc_n, citation.page)}`,
          {
            size: 9,
            color: MUTED,
            gap: 4,
...
function renderContradictions(b: PdfBuilder, data: CaseExportData) {
  ...
  for (const c of items) {
    ...
    if (Object.keys(docA).length || Object.keys(docB).length) {
      ...
      b.text(`"${formatPdfSourceQuote(asStr(docA.quote))}"  — ${citeLabel(docA.doc_n, docA.page)}`, {
        size: 10,
      });
      b.text("Document B:", { size: 9, bold: true, color: ACCENT });
      b.text(`"${formatPdfSourceQuote(asStr(docB.quote))}"  — ${citeLabel(docB.doc_n, docB.page)}`, {
        size: 10,
        gap: 4,
      });
    }
    ...
    if (cites.length) {
      ...
      b.bullets(
        cites.map((cc) => `"${formatPdfSourceQuote(asStr(cc.quote))}"  — ${citeLabel(cc.doc_n, cc.page)}`),
      );
    }
    ...
    if (withQuotes.length) {
      ...
      b.bullets(
        withQuotes.slice(0, 3).map((g) => {
          ...
          return title
            ? `"${formatPdfSourceQuote(asStr(g.quote))}"  — ${title}`
            : `"${formatPdfSourceQuote(asStr(g.quote))}"`;
        }),
      );
    }
    ...
    for (const m of motions) {
      ...
      if (mcites.length) {
        ...
        b.bullets(
          mcites
            .slice(0, 4)
            .map((cc) => `"${formatPdfSourceQuote(asStr(cc.quote))}"  — ${citeLabel(cc.doc_n, cc.page)}`),
        );
      }
      if (m.draft_outline) {
        b.h3("Draft outline");
...
function buildSectionPlan(mode: ReportMode): SectionPlan[] {
  ...
  const sections: SectionPlan[] = [
    ...
    },
    {
      ...
      gatedInLimited: false,
      available: (d) => _citationMode === 'audit' && asArr(asObj(d.report).citations).length > 0,
...
function computeRenderQueue(plan: SectionPlan[], data: CaseExportData, mode: ReportMode) {
  ...
  return plan
    // Citation resolution is mandatory for every materia, even when its
    // optional section profile omits the common evidence-source appendix.
    .filter((s) => s.id === "evidence_sources" || applicable.has(s.id))
    .filter((s) => {
