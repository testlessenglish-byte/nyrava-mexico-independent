...
describe('OpenRouter routing for Legal Analyzing', () => {
  ...
  it('falls back to Groq when the preferred analysis provider has no credits', async () => {
    const attempts: string[] = [];
    vi.mocked(buildProvider).mockImplementation((row) => ({
      type: row.provider_type,
      capabilities: { jsonMode: true, maxContextTokens: 60000 },
      chat: async () => {
        attempts.push(row.provider_type);
        if (row.provider_type === 'openrouter') throw new Error('HTTP 402 insufficient credits');
        return { text: '{"key_findings":[]}', latencyMs: 1, model: 'test-model' };
      },
      testConnection: vi.fn(),
    } as unknown as import("@/lib/ai/providers/types").AIProvider));
    const result = await callGroq({ task: 'analysis', apiKey: 'openrouter-key-test', runtimeProvider: 'openrouter', userId: 'test-user', userContent: 'fallback test', cache: false });
    expect(attempts).toContain('openrouter');
