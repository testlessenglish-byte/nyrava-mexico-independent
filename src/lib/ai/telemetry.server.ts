 ...
 */
const failedScopeCalls = new WeakMap<object, TelemetryCall[]>();
export function failedProviderCalls(error: unknown) {
  const calls = error && typeof error === "object" ? failedScopeCalls.get(error) ?? [] : [];
  return calls.map(call => ({
    provider: call.provider, model: call.model, ok: call.ok,
    input_tokens: call.inputTokens ?? 0, output_tokens: call.outputTokens ?? 0,
...
export async function withTelemetryScope<T>(
  ...
  try {
    ...
    if (error && typeof error === "object") failedScopeCalls.set(error, scope.calls);
    throw error;
  }
}
