  ...
  return withTraceScope(
    ...
    () =>
      withAIUser(userId, async () => {
        try {
          // A failed quality review needs targeted repair. Repeating the same report
          // with invented execution IDs skips prerequisites and self-cancels.
...
async function _runPipelineForCase(
    ...
    if (DONE_STATUSES.has(latestStatusByEngine.get(ENGINE.report) ?? "")) {
      try {
        const { data: reportRow } = await (supabase as any)
          .from("reports")
          .select("updated_at,created_at,citations,execution_id,report_chunk_cache")
          .eq("case_id", caseId)
          .maybeSingle();
        ...
        // A targeted card review can change report inputs without rerunning
        // an upstream engine. Its new log must also invalidate the report cache.
        const { data: latestReview, error: reviewError } = await supabase
          .from("agent_logs").select("finished_at")
          .eq("case_id", caseId).order("finished_at", { ascending: false })
          .limit(1).maybeSingle();
        if (reviewError) throw new Error(reviewError.message);
        const agentReviewStale = Boolean(latestReview?.finished_at &&
          new Date(latestReview.finished_at).getTime() > new Date(reportRow?.updated_at ?? reportRow?.created_at ?? 0).getTime());
        const explicitReportRetry = reportRow?.report_chunk_cache?.__regenerate === true;
        if (missingForExecution || timestampStale || hashStale || agentReviewStale || explicitReportRetry) {
  ...
  if (startFrom) {
    ...
    if (idx >= 0) {
      ...
      const { resumeIndex } = await import("./execution/resume-index");
      const sliceIdx = resumeIndex(idx, firstIncomplete, alreadyDone(stages[idx].key as PipelineStageKey));
      if (sliceIdx !== idx) {
  ...
  try {
    ...
    const finalEngineStates = new Map<string, { status: string; skipped_reason: string | null }>();
    for (const row of finalLedger ?? []) if (!finalEngineStates.has(row.engine)) finalEngineStates.set(row.engine, row);
    ...
    if (
      ...
      finalEngineStates.get("contradictions")?.status !== "completed"
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const { data: postReport } = await (supabase as any)
    .from("reports")
    .select("case_id,quality_blocked,quality_block_reasons")
    .eq("case_id", caseId)
    .maybeSingle();

  const isNeedsRevision =
    !!postReport &&
    (postReport.quality_blocked === true || postRun?.status === "needs_revision" ||
