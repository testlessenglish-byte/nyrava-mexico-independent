...
export async function sweepStalledCases(
  ...
  // Long reports may legitimately run longer than the age cutoff. Only
  // terminate ledger rows belonging to cases whose worker lease expired.
  // Age alone must never kill a live worker and force a subscriber to reset.
  const expiredCaseIds = stalled.map(c => c.id);
  if (!expiredCaseIds.length) return { swept: ids.length, autoRetried, ids };
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ledgerQ: any = opts?.caseId
    ...
    : (db as any)
        ...
        .eq("status", "running")
        .lt("started_at", cutoff);
  const { data: ledgerRows, error: ledgerErr } = await ledgerQ.in("case_id", expiredCaseIds).select("id,case_id");
