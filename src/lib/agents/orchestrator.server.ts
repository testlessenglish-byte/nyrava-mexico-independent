...
async function recordAgent(db: Db, ctx: RunCtx, def: AgentDefinition, startedAt: number, result: AgentResult) {
  ...
  if (error) throw new Error(`Could not save agent ${def.key}: ${error.message}`);
}
...
const agentRunners: Record<string, (c: RunCtx) => Promise<AgentResult>> = {
    intake: agentIntake,
    ocr: agentOcr,
...
export async function runSingleAgentReview(args: OrchestratorArgs, agentKey: string) {
  const def = AGENT_DEFINITIONS.find((agent) => agent.key === agentKey);
  if (!def) throw new Error("Unknown agent");
  const { withAIUser } = await import("@/lib/ai/user-scope.server");
  return withAIUser(args.userId, async () => {
    const { getAnalysisMode } = await import("@/lib/intelligence/evidence-gate.server");
    const ctx: RunCtx = { ...args, runId: crypto.randomUUID(), analysisMode: await getAnalysisMode(args.db, args.caseId) as AnalysisMode };
    const startedAt = Date.now();
    let raw: AgentResult;
    if (agentKey === "orchestrator") {
      // Recheck gate records without launching the other twelve agents.
      const { data, error } = await args.db.from("agent_logs").select("agent_key,status")
        .eq("case_id", args.caseId).order("created_at", { ascending: false });
      if (error) throw new Error(error.message);
      const latest = new Map<string, string>();
      for (const row of data ?? []) if (!latest.has(row.agent_key)) latest.set(row.agent_key, row.status);
      const gates = ["report", "qa", "judge", "hallucination"].map(key => ({ key, status: latest.get(key) ?? "pending" }));
      const ok = gates.every(gate => gate.status === "success");
      raw = { status: ok ? "success" : "blocked", confidence: ok ? 1 : 0, processingTime: 0, tokensUsed: 0,
        outputFile: def.outputFile, errors: ok ? [] : ["Release checks are incomplete or failed."], output: { gates } };
    } else {
      raw = await safeRun(() => agentRunners[agentKey](ctx), def);
    }
    const result = await attachAgentStats(args.db, args.caseId, def, raw, startedAt);
    await recordAgent(args.db, ctx, def, startedAt, result);
