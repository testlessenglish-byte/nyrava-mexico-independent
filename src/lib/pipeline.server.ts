...
${corpusText}`;
  ...
  // Use shared provider rotation and all configured user fallback keys.
...
${digestText}`;
  ...
  if (analyzerSourceConstraint) {
    const { quarantineDispositionConflicts } = await import('./reporting/report-evidence-integrity');
    const audited = quarantineDispositionConflicts(a,analyzerSourceConstraint.disposition,analyzerSourceConstraint.currentNumbers);
    a = audited.data;
    if (audited.rejected.length) console.warn(`[analyzers] rejected ${audited.rejected.length} current-disposition conflicts`);
  }
  // Persist only the grounded category output, not the pre-verification model
  // response that every downstream engine used to read from analyses.
  for (const category of ['contradictions','missing_evidence','procedural_issues','key_findings']) {
    a[category] = groundCat((Array.isArray(a[category]) ? a[category] : []) as Record<string,unknown>[]);
  }
...
async function buildPaginatedCorpus(db: Db, caseId: string) {
  const { data: docs } = await db
    .from("documents")
    .select("id,filename,extracted_text,status,mime_type")
    .eq("case_id", caseId)
    .is("archived_at", null)
    .order("created_at", { ascending: true });
  const extracted = (docs ?? []).filter((d) => d.status === "extracted");
  const { data: sourcePages, error: pageError } = await db.from("document_pages")
    .select("document_id,page,text").eq("case_id", caseId).order("page");
  if (pageError) throw new Error(`Source pages unavailable: ${pageError.message}`);
  const docIndex: { doc_n: number; document_id: string; filename: string; pages: number }[] = [];
  const blocks: string[] = [];
  extracted.forEach((d, i) => {
    const docN = i + 1;
    const storedPages = (sourcePages ?? []).filter(p => p.document_id === d.id);
    if (d.mime_type === "application/pdf" && !storedPages.length) {
      throw new Error(`SOURCE_PAGES_MISSING: ${d.filename}. Re-extract the document before generating its report.`);
    }
    const pages = storedPages.length ? storedPages : [{ page: 1, text: d.extracted_text ?? "" }];
    docIndex.push({
      doc_n: docN,
...
export async function runReport(args: {
  ...
  const { data: sourceIdentityCase } = await db.from("cases").select("matter_metadata").eq("id", caseId).maybeSingle();
  const identityMetadata = (sourceIdentityCase?.matter_metadata as any) ?? {};
  const sourceIdentityAudit = await loadSourceMatterAudit(db, caseId,
...
async function _runReportInner(args: {
  ...
  sourceIdentityAudit: Awaited<ReturnType<typeof import("./intelligence/source-matter-audit.server").loadSourceMatterAudit>>;
}) {
  const { db, caseId, userId, apiKey, apiKeys, forceFinalize, executionId, sourceIdentityAudit } = args;
  ...
  const proceduralPosture = detectProceduralPosture({
    caseRow: caseTsRow,
    corpusText: (await db.from('documents').select('extracted_text').eq('case_id', caseId).is('archived_at', null)).data?.map(d => d.extracted_text ?? '').join('\n\n') ?? '',
  ...
  if (materiaForReport === "migratorio" && mandatoryDecisionCoreRequired) {
    ...
    const dispositionPages = await loadCaseSourcePages(db, caseId);
    migratorioDisposition = resolveMigratorioDisposition((dispositionDocuments ?? []).map(doc=>({...doc,pages:dispositionPages.filter(page=>page.document_id===doc.id)})), mandatoryDecisionCore);
    mandatoryDecisionCore = applyMigratorioDisposition(mandatoryDecisionCore, migratorioDisposition);
    // Correct the persisted/rendered reference, not only a temporary audit copy.
...
${paginationTail}`;
  ...
  const { loadCaseSourcePages: loadContradictionSourcePages } = await import('./intelligence/source-matter-audit.server');
  const { verifyContradictionPairs } = await import('./reporting/report-evidence-integrity');
  const contradictionSourcePages = await loadContradictionSourcePages(db,caseId);
  const contradictions = verifyContradictionPairs(contradictionsRaw,contradictionSourcePages,docIndex).accepted;
  ...
  if (mandatoryDecisionCoreRequired && (String(prose.executive_summary ?? "").trim().length < 80 ||
      !validateMandatoryDecisionCore(mandatoryDecisionCore, {executiveSummary:String(prose.executive_summary ?? ""),findings:[]}).ok)) {
    const { groundedDecisionSummary } = await import("./intelligence/decision-summary");
    const groundedSummary = groundedDecisionSummary(mandatoryDecisionCore, docIndex);
    if (groundedSummary.length >= 80) {
      prose.executive_summary = groundedSummary;
      (parsed as Record<string, unknown>).executive_summary_source = "verified_decision_passages";
    }
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const contradictionPairAudit = verifyContradictionPairs(contradictionsGuarded.items as any[],contradictionSourcePages,docIndex);
  const allContradictions = contradictionPairAudit.accepted;
  ...
  const reportCaseStrengthScore = gatedScore(reconcileCaseStrengthScore(
    reportCaseStrengthScoreRaw ?? reportDeterministicStrength,
