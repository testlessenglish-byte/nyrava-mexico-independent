...
type Document = { id: string; extracted_text: string | null; pages?: MatterSourcePage[] };
...
function issuingCourt(text: string): Court | null {
  ...
  if (/As[ií]\s+lo\s+resolvi[oó]\s+(?:la\s+(?:Primera|Segunda)\s+Sala|el\s+Pleno)\s+de\s+la\s+Suprema\s+Corte\s+de\s+Justicia\s+de\s+la\s+Naci[oó]n/i.test(text)) return "scjn";
  return null;
}
...
function finalOrders(doc: Document): { court: Court | null; date: string | null; items: MandatoryDecisionCoreItem[] } | null {
  ...
  if (doc.pages?.length) {
    const orders = extractOperativeOrders(doc.pages);
    if (orders.length) return { court: issuingCourt(text), date: text.slice(0,2500).match(/Fecha\s+de\s+sentencia\s*:\s*(\d{4}-\d{2}-\d{2})/i)?.[1] ?? null,
      items: orders.map((order,index)=>({ id:`migratorio-final:${doc.id}:page-${order.page}-${index}`,
        kind:"DISPOSITION", text:order.text, speaker_role:issuingCourt(text), proposition_type:"procedural_fact", adoption_status:"adopted",
        source_refs:[{document_id:doc.id,quote:order.text,label:`p.${order.page}`,page:order.page}] })) };
  }
...
export function applyMigratorioDisposition(core: MandatoryDecisionCoreItem[], resolved: MigratorioDisposition): MandatoryDecisionCoreItem[] {
  ...
  // Replace the disposition lane, including reconstruction aliases and orders
