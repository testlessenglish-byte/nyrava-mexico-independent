...
import { mxProfileOrNull, type MxPipelineProfile } from "../execution/mx-pipeline";
...
export type Fuero = "federal" | "comun" | "mixto" | "unresolved";
...
export type JurisdictionProfile = {
  ...
  court_jurisdiction: { level: "federal" | "state" | "municipal" | "unresolved"; entity: string | null };
  applicable_law: { substantive_materia: MxPipelineProfile | null; procedural_materia: MxPipelineProfile | null; local_state: string | null; status: "pending_verification" };
...
/** Court selection and law candidates are independent, auditable dimensions. */
export function buildJurisdictionProfile(args: {
  ...
  const materia = mxProfileOrNull(args.caseType);
  const underlying = mxProfileOrNull(args.underlyingMateria);
  const amparo = materia === "amparo" || /^amparo_|^revision_amparo|^recurso_revision_amparo/.test(args.proceduralVehicle ?? "");
  const proceduralMateria = amparo ? "amparo" : materia;
  const substantiveMateria = amparo ? underlying ?? (materia !== 'amparo' ? materia : null) : materia;
  const law = materia ? MATERIA_LAW[materia] : null;
  const substantiveLaw = substantiveMateria ? MATERIA_LAW[substantiveMateria] : null;
  const proceduralLaw = proceduralMateria ? MATERIA_LAW[proceduralMateria] : null;
  const declaredLevel = jurisdictionLevelOf(args.jurisdictionField);
  // Geography in a quoted precedent or party address is not a jurisdiction fact.
  const state = detectState(args.applicableState);
  const state_source: JurisdictionProfile["state_source"] = state
    ? "case_field" : "unresolved";
  const ctx = resolveLegalContext({materia: proceduralMateria, jurisdictionValue:args.jurisdictionField,
    court:args.issuingCourt, state:state?.name});
  const jurisdiction_level = ctx.jurisdiction_level;
  const jurisdiction_source: JurisdictionProfile["jurisdiction_source"] = declaredLevel ? "declared"
    : jurisdiction_level === "unresolved" ? "unresolved" : args.issuingCourt ? "corpus" : "materia";
  const fuero: Fuero = jurisdiction_level === "federal" ? "federal" : jurisdiction_level === "state" || jurisdiction_level === "municipal" ? "comun" : "unresolved";
  const expand = (codes: readonly string[]) => [...new Set(codes.map(c => c.replace(/%STATE%/g, state?.name ?? "entidad pendiente de determinar")))];
  const substantive_codes = expand(substantiveLaw?.substantive ?? []);
  const procedural_codes = expand(proceduralLaw?.procedural ?? []);
  const courts = expand(proceduralLaw?.courts ?? []);
  const notes = ["Instrumentos y autoridades candidatos: confirmar competencia, disposición vigente y aplicabilidad por procedimiento y fecha antes de utilizarlos como fundamento."];
  if (!materia) notes.push("Materia pendiente de confirmar; no se sustituye por civil.");
  if (jurisdiction_level === "unresolved") notes.push("Competencia pendiente: la ubicación del caso no determina fuero federal, local o concurrente.");
  if (!state && (substantiveLaw?.substantive.some(c => c.includes("%STATE%")) || law?.fuero !== "federal")) notes.push("Entidad aplicable pendiente; no se utiliza CDMX ni legislación federal como sustituto automático.");
  if (amparo && !substantiveMateria) notes.push("Amparo: falta confirmar materia y legislación del acto subyacente.");
  if (state) notes.push(`Configuración local ${state.name}: pendiente de validación específica; el catálogo no acredita soporte jurídico completo.`);
  if ([...substantive_codes,...procedural_codes].some(c => /CNPCF|Código Nacional de Procedimientos Civiles/i.test(c))) notes.push("CNPCF: resolver declaratorias y reformas, tipo de procedimiento, fecha de inicio y transitorios; no basta la publicación del código.");
  const summary = `Materia ${materia ? MATERIA_LABEL_ES[materia] : "pendiente"}; competencia ${jurisdiction_level}; entidad ${state?.name ?? "pendiente"}. Marco candidato sujeto a verificación de aplicabilidad.`;
  return {country:"MX", state, state_source, fuero, jurisdiction_level, jurisdiction_source, materia,
    court_jurisdiction:{level:jurisdiction_level,entity:detectState(args.jurisdictionField)?.code ?? detectState(args.issuingCourt)?.code ?? null},
    applicable_law:{substantive_materia:substantiveMateria,procedural_materia:proceduralMateria,local_state:state?.code ?? null,status:"pending_verification"},
    courts, substantive_codes, procedural_codes, constitutional_basis:materia ? CONSTITUTIONAL_BASIS[materia] : [], summary, notes};
}
