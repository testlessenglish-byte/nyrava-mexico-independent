...
describe('source citation contract',()=>{
 ...
 it('retains a verified physical label for a repeated operative order',()=>{
  ...
  const refs=relocateSourceRefs([{document_id:'a',page:31,label:'p.31',page_located:31,quote}],pages,[]);
  expect(refs[0].page).toBe(31);expect(auditSourceLocations(refs,pages,[]).ok).toBe(true);
 });
 it('does not infer a document from a quote-only reference even at one exact source location',()=>{
  ...
  expect(refs).toEqual([{quote}]);expect(auditSourceLocations(refs,pages,[]).ok).toBe(false);
