...
describe("Migratorio final disposition", () => {
  ...
  it("replaces a reconstruction alias of the same order and remains idempotent", () => {
    ...
    const alias = { ...resolved.items[0], id: "reconstruction-alias", speaker_role: null };
    const anchored = resolveMigratorioDisposition([doc], [alias]);
    expect(anchored.history).toEqual([]);
    const core = applyMigratorioDisposition([alias], anchored);
    expect(core).toEqual(anchored.items);
    expect(applyMigratorioDisposition(core, anchored)).toEqual(core);
    expect(matchesMigratorioDisposition(anchored, core)).toBe(true);
  });
