...
export function detectProceduralPosture(input: {
  ...
  const text = normalize(input.corpusText ?? "");
  const resolutivos = normalize(input.resolutivos ?? "");
  const caption = text.slice(0, 1200);
  ...
  if (/amparo directo en revision/i.test(caption)) {
    decisionType = "ejecutoria_amparo_directo_en_revision";
  } else if (/amparo en revision/i.test(caption)) {
    decisionType = "ejecutoria_amparo_en_revision";
  } else if (/amparo directo/i.test(caption)) {
    decisionType = "sentencia_amparo_directo";
  } else if (/amparo indirecto/i.test(caption)) {
