...
function ground(doc: DocInput, quote: string): SourceReference | null {
  ...
  if (doc.pages?.length) {
    for (const p of doc.pages) if (locateQuoteInText(quote, p.text)) return {
      document_id: doc.id, filename: doc.filename, page: p.page, quote: quote.trim(),
    };
    return null;
  }
...
const PENAL_UNDERLYING_PATTERN =
  /\b(materia\s+penal|proceso\s+penal|causa\s+penal|juicio\s+penal|sentencia\s+penal|averiguaci[óo]n\s+previa|carpeta\s+de\s+investigaci[óo]n|tribunal\s+de\s+enjuiciamiento|juez\s+de\s+control|persona\s+sentenciada)\b/gi;
...
export function classifyCaseFromDocuments(docs: DocInput[]): CaseClassificationResult {
  ...
  // into one blob first.
  {
    ...
    for (const doc of docs) {
      if (!doc.extracted_text) continue;
      const immigrationContext = /procedimiento\s+administrativo\s+migratorio|detenci[oó]n\s+migratoria|agentes?\s+migratorios?|condici[oó]n\s+de\s+refugiado|residencia\s+(?:temporal|permanente)|regularizaci[oó]n\s+migratoria/i.exec(doc.extracted_text.slice(0, 8000));
      if (immigrationContext) {
        perDoc.push({ value: "migratorio", doc, matched: [immigrationContext[0]] });
        continue;
      }
  ...
  const substantiveField = fields.find(f => f.field === "case_type" && f.status === "CONFIRMED");
  fields.push(substantiveField?.value === "migratorio"
    ? { ...substantiveField, field: "underlying_materia" }
    : classifyByPattern("underlying_materia", docs, PENAL_UNDERLYING_PATTERN, () => "penal", "header"));
...
export async function runCaseClassification(
  ...
  for (const doc of docs) {
    ...
    if (error) throw new Error(`Classification source pages unavailable: ${error.message}`);
