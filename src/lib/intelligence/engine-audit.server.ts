...
export async function runEngine<T>(
  ...
  try {
    ...
    const runtime = Date.now() - t0;
    const { failedProviderCalls } = await import("../ai/telemetry.server");
    const calls = failedProviderCalls(e);
    const failureTelemetry = calls.length ? {
      provider: Array.from(new Set(calls.map(call => call.provider))).join(", "),
      meta: { telemetry: { provider_calls: calls } },
    } : {};
