...
const DECISION_RECONSTRUCTION_TIMEOUT_MS = 60_000;
...
export async function ensureDecisionReconstruction(
  ...
  if (failedAfterExtraction && !retryFailed) {
    // A transient provider timeout must not disable this required stage forever.
    // Back off and cap retries for this extracted source version.
    if (Date.now() - latestCreatedAt < 60_000) return null;
    const { count } = await (db as any).from("case_decision_reconstructions")
      .select("id", { count: "exact", head: true }).eq("case_id", caseId)
      .eq("matter_identity_status", "extraction_failed").gte("created_at", caseRow!.extracted_at);
...
export async function buildDecisionReconstruction(
  ...
  const { decisionExcerpt } = await import("./decision-excerpt");
  const corpusText = docs
    .map(
      (d, i) =>
        `=== DOC ${i + 1} | ${d.filename} ===\n${decisionExcerpt(d.extracted_text ?? "")}`,
    )
    .join("\n\n");
...
${corpusText}`,
  ...
  let reconstruction = emptyReconstruction(caseId, new Date().toISOString());
