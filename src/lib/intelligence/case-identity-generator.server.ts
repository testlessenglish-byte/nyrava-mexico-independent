...
export function extractCaseNumbers(text: string): CaseNumberInfo[] {
  const hits: Array<CaseNumberInfo & { priority: number; offset: number }> = [];
    ...
    for (const m of text.matchAll(re)) {
      ...
      if (!seenNorm.has(fullLabel)) {
        ...
        hits.push({
          ...
          offset: m.index ?? Number.MAX_SAFE_INTEGER,
        });
      }
    }
  }

  // A cited precedent later in a judgment cannot replace its opening caption.
  // Specificity only breaks ties at the same position (ADR versus AD).
  hits.sort((a, b) => a.offset - b.offset || b.priority - a.priority);

  if (hits.length > 0) {
    hits[0].isPrimary = true;
  }

  return hits.map(({ priority: _, offset: __, ...rest }) => rest);
}
...
export function generateAutomaticCaseDescription(
  ...
  if (isConcluded) {
    const decisionLabels: Record<string, string> = {
      ejecutoria_amparo_en_revision: "ejecutoria de amparo en revisión",
      ejecutoria_amparo_directo_en_revision: "ejecutoria de amparo directo en revisión",
      sentencia_amparo_directo: "sentencia de amparo directo",
      sentencia_amparo_indirecto: "sentencia de amparo indirecto",
      sentencia_definitiva: "sentencia definitiva",
    };
    const decision = decisionLabels[posture?.decision_type ?? ""] || "resolución judicial";
...
export async function applyAutomaticCaseIdentity(
  ...
  const updatedIdentity: CaseIdentityMetadata = {
    ...
    case_number: primaryNumber?.raw ?? null,
    case_number_normalized: primaryNumber?.normalized ?? null,
    case_number_type: primaryNumber?.type ?? null,
    primary_party_name: partyExtraction.primary ?? null,
    primary_party_role: partyExtraction.primaryRole ?? null,
    opposing_party_name: partyExtraction.opposing ?? null,
    court_name: courtField ?? null,
    tribunal_level: posture?.court_level ?? null,
    procedural_vehicle: vehicleField ?? (caseRow as any)?.procedural_vehicle ?? null,
    effective_materia: materiaField ?? null,
    underlying_materia: underlyingField ?? null,
    jurisdiction: jurisdictionField ?? null,
    procedural_posture: posture?.case_status ?? null,
    related_case_numbers: relatedNumbers,
