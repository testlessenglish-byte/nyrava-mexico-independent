...
export async function addFindings(db: Db, rows: NewFinding[]) {
  ...
  for (const [caseId, group] of byCase) {
    ...
    if (area === 'migratorio') {
      ...
      const source = await loadReportSourceConstraint(db,caseId);
      if (source) {
        const audit = quarantineDispositionConflicts(policyKept,source.disposition,source.currentNumbers);
        sourceChecked = audit.data;
        if (audit.rejected.length) console.warn(`[source-disposition] case=${caseId} rejected ${audit.rejected.length} conflicting assertions`);
      }
    }
    const { kept } = await validateFindingsForCase(db, caseId, sourceChecked);
