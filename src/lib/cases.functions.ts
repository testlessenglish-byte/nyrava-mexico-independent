...
async function runLabeledStep({
  ...
  const stepProvider: import("@/lib/ai-keys.server").ProviderName = "groq";
...
export const getCase = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const matterIdentity = ((c.data?.matter_metadata as any)?.case_identity ?? {}) as Record<string, any>;
    const sourceIdentityAudit = await loadSourceMatterAudit(supabase, data.caseId,
      matterIdentity.case_number_normalized ?? matterIdentity.case_number);
    const identityBlocked = sourceIdentityAudit.status === "IDENTITY_CONFLICT";
    const checkedReport = report.data && identityBlocked ? {
