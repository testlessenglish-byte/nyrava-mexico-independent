  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          switch (event.type) {
            ...
            case "customer.subscription.deleted": {
              ...
              await admin.from("subscriptions").update({ status: "canceled" }).eq("user_id", userId).throwOnError();
              await provisionOrganizationSubscription(admin, {
                eventId: event.id,
                eventType: event.type,
                userId,
            ...
            case "invoice.payment_failed": {
              ...
              await admin.from("subscriptions").update({ status: "past_due" }).eq("user_id", userId).throwOnError();
              await provisionOrganizationSubscription(admin, {
                eventId: event.id,
                eventType: event.type,
                userId,
          ...
          // A failed write must be retried by Stripe; logging is not fulfillment.
        }
        ...
        return new Response(JSON.stringify(logStatus === "error" ? { error: "subscription processing failed" } : { received: true }), {
