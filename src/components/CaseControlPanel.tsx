...
export function CaseControlPanel({
  ...
  const [classificationConfirmed, setClassificationConfirmed] = useState(
    matterMetadata?.classification_confirmed === true,
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-primary/20 bg-background/60 p-4">
        ...
        <button
          onClick={() => {
            if (caseType === "migratorio" && !classificationConfirmed) {
            ...
            <div className="w-full max-w-md rounded-2xl border border-primary/30 bg-card p-6 shadow-2xl space-y-4">
              <div className="border-b border-border pb-3">
                <span className="text-[10px] font-bold tracking-widest text-primary uppercase">
                  {locale === "es" ? "Revisión Previa al Análisis" : "Pre-Analysis Verification"}
                </span>
              ...
              <div className="space-y-2.5 text-xs">
                <div className="flex justify-between border-b border-border/40 py-1">
                  <span className="text-muted-foreground font-medium">Materia:</span>
                  <span className="font-semibold text-foreground">Derecho Migratorio, Refugio y Nacionalidad</span>
                </div>
                <div className="flex justify-between border-b border-border/40 py-1">
                  ...
                  <span className="font-medium text-foreground text-right max-w-[220px]">
                    {String(matterMetadata?.immigration_subtype_label_es || matterMetadata?.immigration_subtype || "Auto-detectado desde documentos")}
                  </span>
                </div>
                <div className="flex justify-between border-b border-border/40 py-1">
                  <span className="text-muted-foreground font-medium">Procedimiento:</span>
                  <span className="font-medium text-foreground text-right">
                    {String(matterMetadata?.detected_procedural_posture || "Procedimiento Administrativo Migratorio")}
                  </span>
                </div>
                <div className="flex justify-between border-b border-border/40 py-1">
                  <span className="text-muted-foreground font-medium">Autoridad:</span>
                  <span className="font-medium text-foreground text-right">
                    {String(matterMetadata?.detected_authority || "Instituto Nacional de Migración (INM)")}
                  </span>
                </div>
                <div className="flex justify-between border-b border-border/40 py-1">
                  <span className="text-muted-foreground font-medium">Jurisdicción:</span>
                  <span className="font-medium text-foreground">Federal</span>
                </div>
                <div className="flex justify-between border-b border-border/40 py-1">
                  <span className="text-muted-foreground font-medium">Estado procesal:</span>
                  <span className="font-medium text-foreground">
                    {String(matterMetadata?.procedural_stage || "En trámite")}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground font-medium">Confianza:</span>
                  <span className="inline-flex items-center rounded-full bg-emerald-500/15 px-2 py-0.5 text-[11px] font-semibold text-emerald-400">
                    {typeof matterMetadata?.immigration_subtype_confidence === "number" && matterMetadata.immigration_subtype_confidence < 0.6
                      ? "Media"
                      : "Alta"}
                  </span>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 rounded-lg border border-border bg-secondary/60 px-3 py-2 text-xs font-semibold text-foreground hover:bg-secondary"
                >
                ...
                >
                  {locale === "es" ? "Confirmar y analizar" : "Confirm & analyze"}
                </button>
              </div>
            </div>
          </div>
