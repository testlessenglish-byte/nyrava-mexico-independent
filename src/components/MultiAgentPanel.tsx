...
import { runMultiAgentAnalysis, getAgentLogs, rerunSingleAgent } from "@/lib/agents/multi-agent.functions";
...
export function MultiAgentPanel({ caseId, report, isProcessing = false }: { caseId: string; report?: unknown; isProcessing?: boolean }) {
  const qc = useQueryClient();
  const { t } = useI18n();
  const runFn = useServerFn(runMultiAgentAnalysis);
  const singleFn = useServerFn(rerunSingleAgent);
  ...
  const singleMut = useMutation({
    mutationFn: (agentKey: string) => singleFn({ data: { caseId, agentKey } }),
    onMutate: () => setRunning(true),
    onSuccess: (res) => {
      setRunning(false);
      void qc.invalidateQueries();
      if (res.result.status === "failed" || res.result.status === "blocked") {
        toast.warning(t("agents.single.failed"));
      } else {
        toast.success(t(res.reportQueued ? "agents.single.queued" : "agents.single.unchanged"));
      }
    },
    onError: (error) => { setRunning(false); toast.error(error instanceof Error ? error.message : t("agents.toast.failed")); },
  });
  ...
  return (
    <Card className="border-primary/15 bg-background/60">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        {isAdmin ? (
          <Button onClick={() => mut.mutate()} disabled={isProcessing || running || mut.isPending} size="sm" title={t("agents.rerun.hint")}>
              ...
              >
                ...
                <div className="min-w-0 flex-1 overflow-hidden">
                  ...
                  {isAdmin && (
                    <button
                      type="button"
                      onClick={() => singleMut.mutate(def.key)}
                      disabled={isProcessing || running || mut.isPending}
                      ...
                      title={t("agents.single.hint")}
                      aria-label={`${t("agents.single.run")}: ${t(`agent.${def.key}.name`)}`}
                    >
                      {singleMut.isPending && singleMut.variables === def.key ? (
                        <Loader2 className="h-3 w-3 animate-spin" />
                      ) : (
                        <Play className="h-3 w-3" />
                      )}
                      {singleMut.isPending && singleMut.variables === def.key ? t("agents.running") : t("agents.single.run")}
                    </button>
                  )}
                </div>
              </div>
